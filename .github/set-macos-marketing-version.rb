...
# Updates only the build configurations belonging to the macOS DynoPrompt target.
# Keeping this scoped prevents a macOS release tag from changing the independent
# marketing version of the DynoPromptiOS app and its test bundle.
...
project_path = ARGV[1] || File.expand_path(
  "../DynoPrompt/DynoPrompt.xcodeproj/project.pbxproj",
  __dir__
)
...
target = native_targets.match(
  /^\s*[0-9A-F]+ \/\* DynoPrompt \*\/ = \{(?<body>.*?)^\s*\};/m
)
abort 'macOS target "DynoPrompt" not found' unless target

configuration_list_id = target[:body][
  /buildConfigurationList = ([0-9A-F]+) \/\* Build configuration list for PBXNativeTarget "DynoPrompt" \*\//,
  1
]
...
puts "Set macOS DynoPrompt marketing version to #{version} in #{configuration_ids.length} configurations."

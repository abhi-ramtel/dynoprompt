...
/// The only command the read-only remote view may send.
...
class BrowserServer {
    ...
    /// Connections that have presented the session token. Only these receive
    /// script or transcript frames.
    private var authenticatedConnections: Set<ObjectIdentifier> = []
    private var broadcastTimer: Timer?

    /// Per-session shared secret. Regenerated on every start, embedded in the
    /// page we serve, and required as the first WebSocket frame.
    private(set) var authToken: String = ""

    /// Bounds resource use from a hostile or buggy client on the LAN.
    ...
    var connectedClients: Int { authenticatedConnections.count }

    // MARK: - Lifecycle

    func start() {
        stop()
        authToken = LocalServerSecurity.generateToken()
    ...
    private func handleHTTPConnection(_ conn: NWConnection) {
        ...
        conn.receive(
            ...
            maximumLength: LocalServerSecurity.maxRequestBytes
        ) { [weak self] data, _, _, error in
            guard let self else { conn.cancel(); return }
            guard error == nil, let data else { conn.cancel(); return }

            let raw = String(decoding: data, as: UTF8.self)
            let response = LocalServerSecurity.rejectionResponse(
                forRequest: raw,
                port: self.httpPort,
                allowedOriginPorts: [self.httpPort, self.wsPort]
            ) ?? self.buildHTTPResponse()
    ...
    private func buildHTTPResponse() -> Data {
        LocalServerSecurity.pageResponse(
            html: Self.generateHTML(wsPort: wsPort, authToken: authToken)
        )
    }
    ...
    private func startWSListener() {
        ...
        let wsOptions = LocalServerSecurity.websocketOptions(
    ...
    private func handleWSConnection(_ conn: NWConnection) {
        ...
        // A client that does not authenticate promptly is dropped, so an
        // unauthenticated socket cannot sit and wait for a future leak.
        let connectionID = ObjectIdentifier(conn)
        DispatchQueue.main.asyncAfter(deadline: .now() + 5) { [weak self] in
            guard let self else { return }
            if !self.authenticatedConnections.contains(connectionID) {
                conn.cancel()
            }
        }
    ...
    private func receiveWSMessage(_ conn: NWConnection) {
        conn.receiveMessage { [weak self] data, _, _, error in
            ...
            if let data { self?.handleIncomingMessage(data, from: conn) }
            self?.receiveWSMessage(conn)
        }
    }

    /// The remote view is read-only, so the only message it may send is the
    /// handshake. Anything else closes the connection.
    private func handleIncomingMessage(_ data: Data, from conn: NWConnection) {
        let connectionID = ObjectIdentifier(conn)
        guard !authenticatedConnections.contains(connectionID) else { return }

        guard data.count <= 4096,
              let command = try? JSONDecoder().decode(BrowserCommand.self, from: data),
              command.type == "auth",
              let token = command.text,
              !authToken.isEmpty,
              LocalServerGuard.secureCompare(token, authToken) else {
            conn.cancel()
            return
        }
    ...
    private func broadcastCurrentState() {
        guard contentActive, !authenticatedConnections.isEmpty else { return }
    ...
    private func broadcast(_ state: BrowserState) {
        let recipients = wsConnections.filter {
            authenticatedConnections.contains(ObjectIdentifier($0))
        }
        guard !recipients.isEmpty, let data = try? JSONEncoder().encode(state) else { return }
    ...
    static func generateHTML(wsPort: UInt16, authToken: String) -> String {
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        <div id="waiting">
          <div class="icon">📡</div>
          <div class="title">Waiting for DynoPrompt…</div>
        ...
        const MIRROR_KEY='dynoprompt.remote.mirror';
        ...
        function connect(){
          ...
          ws.onopen=()=>{clearTimeout(rt);
            /* The server sends nothing until this arrives. */

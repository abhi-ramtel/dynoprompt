...
class DirectorServer {
    ...
    /// Upper bound on a single director command, including the script body.
    static let maxCommandBytes = 512 * 1024

    // Dedicated queue for broadcasting to avoid blocking the main/UI thread
    private let broadcastQueue = DispatchQueue(label: "com.dynoprompt.director.broadcast")
    ...
    func start() {
        ...
        // A server with no token would accept every client; refuse to run.
    ...
    private func handleHTTPConnection(_ conn: NWConnection) {
        ...
        conn.receive(
            ...
            // This page embeds the session token, so serving it to a request
            // that arrived under an attacker-controlled hostname would hand
            // that token to a DNS-rebinding page.
    ...
    private func handleIncomingMessage(_ data: Data, from conn: NWConnection) {
        // Bound the script a director can push. Beyond this the overlay would
        // stall laying out text rather than prompt anyone.
        guard data.count <= DirectorServer.maxCommandBytes else {
            conn.cancel()
            return
        }
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if !self.authenticatedConnections.contains(connId) {
                if command.type == "auth",
                   ...
                   LocalServerGuard.secureCompare(token, self.authToken) {
    ...
    static func generateHTML(wsPort: UInt16, authToken: String) -> String {
        ...
        <title>DynoPrompt Director</title>
        <style>

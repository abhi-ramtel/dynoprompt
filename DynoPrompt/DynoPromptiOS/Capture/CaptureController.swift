...
enum CaptureIssue: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .cameraPermissionDenied:
            "Camera access is turned off for DynoPrompt."
        case .microphonePermissionDenied:
            "Microphone access is turned off for DynoPrompt."
        case .cameraUnavailable:
    ...
    AVCaptureFileOutputRecordingDelegate, @unchecked Sendable
{
    ...
    @ObservationIgnored nonisolated private let sessionQueue = DispatchQueue(
        label: "dev.fka.dynoprompt.ios.capture.session",
        qos: .userInitiated
    )
    @ObservationIgnored nonisolated private let sampleQueue = DispatchQueue(
        label: "dev.fka.dynoprompt.ios.capture.samples",
        qos: .userInitiated
    )
    ...
    private func beginBackgroundTaskIfNeeded() {
        ...
        backgroundTaskID = UIApplication.shared.beginBackgroundTask(withName: "Finish DynoPrompt Recording") {
    ...
    nonisolated private static func makePendingRecordingURL() throws -> URL {
        ...
        return baseURL
            .appendingPathComponent("DynoPrompt-\(UUID().uuidString)")

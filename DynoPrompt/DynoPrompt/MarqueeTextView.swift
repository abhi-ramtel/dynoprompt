...
struct WordFlowLayout: View {
    ...
    //
    // Starts from the font's own metrics — a face with a large built-in line
    // height (OpenDyslexic) already supplies most of the gap, so it needs less
    // added — then scales by the user's line-spacing setting. At the default
    // multiplier this reproduces the previous fixed behaviour exactly, so
    // existing setups look unchanged.
    private var lineSpacing: CGFloat {
        let intrinsicHeight = font.ascender - font.descender + font.leading
        let ratio = intrinsicHeight / font.pointSize
        let base: CGFloat = ratio > 1.5 ? 2 : 8
        let multiplier = NotchSettings.shared.lineSpacingMultiplier
            / NotchSettings.defaultLineSpacing

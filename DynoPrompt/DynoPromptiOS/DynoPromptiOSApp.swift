...
@main
struct DynoPromptiOSApp: App {
    @State private var model = AppModel()

    var body: some Scene {
        WindowGroup {
            #if DEBUG
            if ProcessInfo.processInfo.arguments.contains("-DynoPromptGalleryPreview") {
                SavedRecordingsGallery(store: Self.galleryPreviewStore())
                    .preferredColorScheme(.dark)
            } else if ProcessInfo.processInfo.arguments.contains("-DynoPromptSettingsPreview") {
                SettingsView(model: model)
                    .preferredColorScheme(.dark)
            } else if ProcessInfo.processInfo.arguments.contains("-DynoPromptPromptPreview") {
    ...
    private static func promptPreviewConfiguration() -> PromptSessionConfiguration {
        PromptSessionConfiguration(
            ...
            DynoPrompt follows every word you say and keeps the last spoken line close to the camera.

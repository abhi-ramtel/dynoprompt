...
class NotchOverlayController: NSObject {
    ...
    func show(text: String, hasNextPage: Bool = false, onComplete: (() -> Void)? = nil) {
        ...
        self.onNextPage = {
            DynoPromptService.shared.advanceToNextPage()
        }
    ...
    private func installKeyMonitor() {
        ...
        escMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            ...
            guard let command = PrompterShortcutResolver.command(
                forKeyCode: event.keyCode,
                characters: event.charactersIgnoringModifiers,
                modifiers: event.modifierFlags
            ) else { return event }

            // Esc closes the page picker first, so it stays an "undo one
            // level" key rather than always killing the session.
            if command == .stop, self.overlayContent.showPagePicker {
    ...
    /// Applies a keyboard command to the running session.
    ///
    /// Manual commands always win over the speech matcher: if the presenter
    /// says they are somewhere, they are.
    func perform(_ command: PrompterCommand) {
        let settings = NotchSettings.shared
        ...
        case .forwardSentence:
            stepSentence(forward: true)

        case .backSentence:
            stepSentence(forward: false)

        case .reset:
            speechRecognizer.jumpTo(charOffset: 0)

        case .increaseFont:
            // Steps the continuous value rather than hopping between presets,
            // so the size can be dialled in mid-session.
            settings.fontSize += 2
        ...
        case .increaseWidth:
            settings.notchWidth += 40

        case .decreaseWidth:
            settings.notchWidth -= 40

        case .increaseHeight:
            settings.textAreaHeight += 30

        case .decreaseHeight:
            settings.textAreaHeight -= 30

        case .increaseOpacity:
            settings.overlayTransparencyOpacity = min(1.0, settings.overlayTransparencyOpacity + 0.05)

        case .decreaseOpacity:
            settings.overlayTransparencyOpacity = max(0.2, settings.overlayTransparencyOpacity - 0.05)

        case .nextPage:
            if overlayContent.hasNextPage { onNextPage?() }
        }
    }

    /// Moves the highlight by whole words, using the word boundaries the
    /// overlay already renders so the jump lands where the eye expects.
    private func stepWords(_ delta: Int) {
        let words = overlayContent.words
        guard !words.isEmpty else { return }

        // Character offsets of the end of each word in the collapsed text.
        var offsets: [Int] = []
        var running = 0
        for word in words {
            running += word.count
        ...
        let current = speechRecognizer.recognizedCharCount
        var index = offsets.firstIndex(where: { $0 > current }) ?? offsets.count
        index += delta
        index = max(0, min(index, offsets.count))
        speechRecognizer.jumpTo(charOffset: index == 0 ? 0 : offsets[index - 1])
    }

    /// Moves to the next or previous sentence boundary.
    private func stepSentence(forward: Bool) {
        let words = overlayContent.words
        guard !words.isEmpty else { return }
        ...
        let target: Int?
        if forward {
            target = boundaries.first(where: { $0 > current })
        } else {
            target = boundaries.last(where: { $0 < current })
        }
        speechRecognizer.jumpTo(charOffset: target ?? (forward ? running : 0))
    }
        ...
        Timer.publish(every: 0.1, on: .main, in: .common)
            ...
            .sink { [weak self] _ in
                ...
                if let targetIndex = self.overlayContent.jumpToPageIndex {
                    ...
                    DynoPromptService.shared.jumpToPage(index: targetIndex)
                }
    ...
    private func beginKeepAwakeActivity() {
        ...
        keepAwakeActivity = ProcessInfo.processInfo.beginActivity(
            ...
            reason: "DynoPrompt teleprompter reading session"
        )
    }
